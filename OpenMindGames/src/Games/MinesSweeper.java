...
public abstract class MinesSweeper
{
    ...
    protected int filed[][];
    protected int mask[][];
    private MinesSweeperGameState state;
    ...
    public void LoadGame(int width, int height, int mines)
    {
        ...
        this.computeFiled();
        this.clearMask();
        this.state = MinesSweeperGameState.WaitForStart;
        stopwatch.reset();
    }
    ...
    public void ClickIn(int x, int y)
    {
        if (InRange(x, y))
        {
            if (getState() != MinesSweeperGameState.GameOver && getState() != MinesSweeperGameState.GameWin)
            {
                if(getState() == MinesSweeperGameState.WaitForStart)
                {
                    state = MinesSweeperGameState.InGame;
                    stopwatch.start();
                }
                if (mask[x][y] == UNKNOWN || mask[x][y] == QUESTIONMARK)
                {
                    if (filed[x][y] == MINE)
                    {
                        mask[x][y] = FAIL;
                        FindWrongMineMarks();
                        state = MinesSweeperGameState.GameOver;
                        stopwatch.stop();
                    }
                ...
                if (IsWin())
                {
                    state = MinesSweeperGameState.GameWin;
                    stopwatch.stop();
                }
            }
        }
     ...
     * @return the state
     */
    public MinesSweeperGameState getState()
    {
        return state;
    }
}
